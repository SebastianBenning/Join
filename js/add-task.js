 ...
 * @param {*} i - idintifies from where the task is created contact or in task
 */
async function addToTask(i, taskStatus) {
  if (taskCategoryFinaly.length == 0) {
    ...
    let currentTask = createNewTask(taskStatus);
...
/**
 * 
 * @param {*} i idintifies from where the task created. (0 = from contact side), (1 = form board side)
 */
function lookingSide(i) {
  if (i == 0) {
...
// close the mask of task
function closeAddTaskMaskBG() {
...
function closeAddTaskMask(i) {
  ...
  if (i == 1) {
    document.getElementById('AddTaskMaskBg').classList.add('d-none');
    let closeaddtask = document.getElementById('AddTaskMaskContainer');
    closeaddtask.innerHTML = '';
    selectorcontactIndex = 0;
  }
  else if (i == 0) {
 ...
 * @param {*} contact idintifies from where the task created. (0 = from contact side)
 */
function showUsers(contact) {
  ...
  if (selectorcontactIndex == 0) {
    ...
    document.getElementById('selector_user_dropdown').classList.add('selector-user-dropdown');
...
// show the users
function showAllUsers(contact) {
    document.getElementById('selector_user_dropdown').innerHTML += showInviteNewContact();
  if (contact == 0) {
    selectedUserfromContact();
  }
  if (userSelect.length > 0) {
    selectedUserfromTask();
  }
}


// shows all contacts
function selectedUserfromTask() {
...
// look from which contact the task was made
function selectedUserfromContact() {
...
function showSelectedContactBubbles() {
  document.getElementById('selector_user_dropdown').innerHTML = ``;
  document.getElementById('selector_user_dropdown').classList.remove('selector-user-dropdown');
...
function selectedUser(contactInitials, contactcolor, contactname) {
  ...
  if (document.getElementById('user_select' + contactInitials + contactcolor + contactname).classList.contains('checked')) {
    ...
    userPush(contactInitials, contactcolor, contactname);
    document.getElementById('user_select' + contactInitials + contactcolor + contactname).classList.add('checked');
    document.getElementById('user_select' + contactInitials + contactcolor + contactname).src = 'assets/img/userSelect-selected.png';
  }
}

function userPush(contactInitials, contactcolor, contactname) {
  userSelect.push({
