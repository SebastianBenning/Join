...
/**
 * open create contact
 * @param {*} mo idintifies from where the contact is open. (0 = contact side), (1 = board side)
 */
...
function createContact(mo) {
    ...
    let checkemail = checkEmailInArray(contactTask);
    checkOrLoad(checkemail, contactName, contactTask, mo);
}


/**
 * Check e-mail or create a contact
 * @param {number} checkemail Checks if e-mail is assigned in array
 * @param {string} contactName name of the contact
 * @param {string} contactTask the entire contact with email and number
 * @param {number} mo idintifies from where the contact is open. (0 = contact side), (1 = board side)
 */
function checkOrLoad(checkemail, contactName, contactTask, mo) {
    if (checkemail == -1) {
        ...
        if (mo == 1) {
            createNewLetter(contactName, mo);
        }
    }
    else {
        checkEmail();
 ...
 * @param {*} mo idintifies from where the contact is open. (0 = contact side), (1 = board side)
 */
function createNewLetter(contactName, mo) {
